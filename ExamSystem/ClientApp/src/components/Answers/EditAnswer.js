...
const EditAnswer = () => {
    ...
    const { examId, questionId, id } = useParams();
    ...
    useEffect(() => {
        const loadAnswer = async () => {
            try {
                const response = await axiosPrivate.get(`/exams/${examId}/questions/${questionId}/answers/${id}`);
        ...
        loadAnswer();
    },[examId, axiosPrivate, questionId, id, navigate, location])
    ...
    const handleSubmit = async (event) => {
      ...
      try {
        await axiosPrivate.put(`/exams/${examId}/questions/${questionId}/answers/${id}`, answerData);
        handleGoBack();
      } catch (err) {
        console.error(err);
        navigate('/login', { state: { from: location }, replace: true });
